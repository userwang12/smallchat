...
int socketSetNonBlockNoDelay(int fd) {
    int flags, yes = 1;
    //设置文件描述符非阻塞并且，不需要延迟等待，写缓冲区一有数据直接发送，非延迟发送
...
int TCPConnect(char *addr, int port, int nonblock) {
    ...
    hints.ai_family = AF_UNSPEC;//设置返回ipv4或者ipv6的格式
    hints.ai_socktype = SOCK_STREAM;//设置为tcp的格式
    //获取目标服务端的地址信息，并以指定的格式
    if (getaddrinfo(addr,portstr,&hints,&servinfo) != 0) return -1;
    //servinfo就是传出参数，代表服务端地址信息的链表头指针
    for (p = servinfo; p != NULL; p = p->ai_next) {
        /* Try to create the socket and to connect it.
         * If we fail in the socket() call, or on connect(), we retry with
         * the next entry in servinfo. */
        //如果当前指针指向的地址信息可以连接就能创建套接字，不能就创建套接字失败
        if ((s = socket(p->ai_family,p->ai_socktype,p->ai_protocol)) == -1)
            continue;

        /* Put in non blocking state if needed. */
        //这里写的比较牛，如果nonblock为1那么就直接设置非阻塞非延迟，
        //如果是我写我可能就把设置函数写到里面了，可能就会增大代码量
        ...
        if (connect(s,p->ai_addr,p->ai_addrlen) == -1) {
            ...
            //这里有问题就是他不一定连接成功还有就是，内存servinfo并没有得到释放
            if (errno == EINPROGRESS && nonblock) return s;

            /* Otherwise it's an error. */
            //如果一个servinfo连接失败了那么就会直接跳出for循环，代表不行
            close(s);
            break;
        }
...
int acceptClient(int server_socket) {
    ...
    while(1) {
        //这里使用while的原因是处理accept系统调用被中断的情况，跟SA_RESTART的结果有些类似，但是实现不同，
        //SA_RESTART并不会返回，而是直接回到上次系统调用的地方
        struct sockaddr_in sa;
        socklen_t slen = sizeof(sa);
        s = accept(server_socket,(struct sockaddr*)&sa,&slen);
        if (s == -1) {
            if (errno == EINTR)//EINTR表示系统调用被中断，表示系统调用因为接收到了中断信号而中断
                continue; /* Try again. */
            else
                return -1;
        }
...
void *chatMalloc(size_t size) {
    void *ptr = malloc(size);//malloc默认创建的内存默认就是void*
    if (ptr == NULL) {
